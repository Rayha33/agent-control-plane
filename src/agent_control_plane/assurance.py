...
class Assurance:
    ...
    def ratified_fingerprint(self) -> str | None:
    ...
    def ratify(self) -> dict[str, Any]:
        policy = self.supervisor.assurance_policy
        previous = self.ratified_fingerprint()
        current = policy.fingerprint
        if previous == current:
            return {"fingerprint": current, "changed": False, **policy.describe()}
        self._record_fingerprint(current, previous, "policy.ratified")
        return {
            "fingerprint": current,
            "changed": True,
    ...
    def _record_fingerprint(self, current: str, previous: str | None, event: str) -> None:
        with self.supervisor.connect() as connection:
