...
def test_valid_executable_is_accepted(tmp_path: Path) -> None:
    ...
    repo.mkdir()
    tool = make_executable(tmp_path / "tool.sh")
    assert resolve_trusted_executable(str(tool), repo) == tool.resolve()
...
def test_run_trusted_revalidates_immediately_before_exec(tmp_path: Path) -> None:
    """Validation at config load says nothing about the file being run now."""
    tool = make_executable(tmp_path / "tool.sh")
    assert run_trusted([str(tool)], tmp_path / "wd", {})["exit_code"] == 0
    tool.chmod(0o644)  # exec bit removed after the config-time check
    with pytest.raises(DriverError) as error:
...
def test_run_trusted_does_not_use_a_shell(tmp_path: Path) -> None:
    ...
    tool = make_executable(
        tmp_path / "echo.sh", "#!/bin/sh\nprintf '%s' \"$1\"\n"
    )
    result = run_trusted(
        [str(tool), f"; touch {marker}"], tmp_path / "wd", os.environ.copy()
    )
...
def browser_driver() -> BrowserProfileDriver:
    return BrowserProfileDriver(
        DriverDefinition(
            name="browser", kind="browser_profile", executable=None, options={}
        )
    )
...
def test_compose_project_name_is_deterministic_and_scoped(tmp_path: Path) -> None:
    ...
    driver = DockerComposeDriver(
        DriverDefinition(
            name="stack", kind="docker_compose", executable=tool, options={}
        )
    )
...
def test_build_driver_requires_executable_for_external_kinds() -> None:
    with pytest.raises(DriverError):
        build_driver(
            DriverDefinition(
                name="stack", kind="docker_compose", executable=None, options={}
            )
        )
...
def test_supervisor_sets_up_and_proves_driver_cleanup(driver_repo: Path) -> None:
    ...
    assert resources[0]["ownership_token"], "resource must be bound to the attempt"
    ...
    assert supervisor.quarantined_resources() == []
